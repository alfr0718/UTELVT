...
class EjemplarController extends Controller
{
    ...
    {
        if (($model = Ejemplar::findOne(['id' => $id])) !== null) {
            return $model;
        }
     ...
     * Si la creación es exitosa redirige a la vista del libro.
     * @return string|\yii\web\Response
     */
    public function actionAgregarEjemplar($libro_id, $biblioteca_idbiblioteca)
    {
        $model = new Ejemplar();
        $model->libro_id = $libro_id;

        $model->biblioteca_idbiblioteca = $biblioteca_idbiblioteca;
        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
                Yii::$app->session->setFlash('success', 'Creación Exitosa');

                return $this->redirect(['libro/view', 'id' => $libro_id, 'biblioteca_idbiblioteca' => $biblioteca_idbiblioteca]);
            } else {
                Yii::$app->session->setFlash('error', 'Hubo un fallo');
            }
        } else {
            $model->loadDefaultValues();
        }
