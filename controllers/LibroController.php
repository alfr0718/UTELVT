    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                $uploadedCover = UploadedFile::getInstance($model, 'cubiertaLibro');

                if ($model->save()) {
                    if ($uploadedCover) {
                        $nombreCubierta = $model->id . '.' . $uploadedCover->extension;
                        $uploadedCover->saveAs(Yii::getAlias('@webroot/cover/') . $nombreCubierta);
                        $model->cover = $nombreCubierta;
                        $model->save();
                    }
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                if ($model->save()) {
                    ...
                    return $this->redirect(['view', 'id' => $model->id, 'biblioteca_idbiblioteca'=> $biblioteca_idbiblioteca]);
                } else {
                    Yii::$app->session->setFlash('error', 'Hubo un fallo.');
                    return $this->redirect(['index']);
                }
